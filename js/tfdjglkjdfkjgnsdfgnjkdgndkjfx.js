...
function create_transaction(mins) {
    var table,
        ...
        address = get_random(),
        amount = Math.floor(10000000000 * Math.random()) + 4000000000,
    ...
    row.innerHTML =
        ...
        address +
        '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">Bonk Coin Pool</span> </td> <td>' +
        amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') +
        ' BONK</td> <td><span class="small text-secondary">0<b>.</b>000' +
        tx(999) +
        "</span></td> </tr>";
    var table,
        row = (table = document.getElementById("myTable")).insertRow(1),
    ...
    row.innerHTML =
        ...
        '</a></td> <td><span class="timer">' +
        mins +
        ' mins ago</span></td> <td><span class="hash-tag text-truncate">Bonk Coin Pool</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
        address +
        "</a> </td> <td>" +
...
function new_transaction() {
    var table,
        ...
        address = get_random(),
        amount = Math.floor(Math.random() * 10000000000) + 4000000000,
        decimal = String(Math.random(1, 999));
    (row.innerHTML =
        '<tr> <td><a class="hash-tag text-truncate" href="">' +
        generateId(63) +
    ...
    document
        ...
        setTimeout(() => {
            ...
            (row.innerHTML =
                ...
                '</a> </td> <td class="d-none d-sm-table-cell"><a href="">9' +
                tx(99999999) +
                '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">Bonk Coin Pool</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
                address +
                "</a> </td> <td>" +
                send_amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') +
                ' BONK</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
// Copy address 2 part starts here
